...
'''
@author: Lain
    Thanks Bram Cohen for the Rijndael interface
'''
...
class Rijndael(object):
    ...
    def decrypt(self, ciphertext):
        ...
        return b''.join(map(lambda x: bytes([x]), result))
...
def decrypt(c, k = 'A1dPUcrvur2CRQyl'):
    ...
    m = b''
    while c:
        m += Rijndael(k).decrypt(c[:16])
        c = c[16:]
    return strip_PKCS7_padding(m)

def encrypt(m, k = 'A1dPUcrvur2CRQyl'):
    def append_PKCS7_padding(s):
        numpads = 16 - (len(s) % 16)
        return s + numpads * chr(numpads)

    m = append_PKCS7_padding(m)
    c = ''
    while m:
        c += Rijndael(k).encrypt(m[:16])
        m = m[16:]
    return c
...
def decrypt_file(input_filename, output_filename):
    print (u'解密 %s -> %s' % (basename(input_filename), basename(output_filename)))
...
def encrypt_file(input_filename, output_filename):
    print (u'加密 %s -> %s' % (basename(input_filename), basename(output_filename)))
    f = open(input_filename, 'rb')
    d = encrypt(f.read())
    f.close()
...
if __name__ == '__main__':
    import sys
    programname = basename(sys.argv.pop(0))

    if len(sys.argv) == 0:
        print(u'''解密：
    python %s -d <文件名列表>
    注意会覆盖目标文件。
    例如源文件是 C:\\adb\\adv_chara5123 则解密后的文件保存在 C:\\adb\\adv_chara5123.png，如果文件已存在则会覆盖。
加密：
    python %s -e <文件名列表>
    注意会覆盖目标文件。
    例如源文件是 C:\\adb\\adv_chara5123.png 则加密后的文件保存在 C:\\adb\\adv_chara5123，如果文件已存在则会覆盖。
不带参数选项则默认是解密。
''' % (programname, programname))
        exit()

    do_method = lambda n: decrypt_file(n, '%s.png' % n)
    if sys.argv[0] == '-d':
        sys.argv.pop(0)
    elif sys.argv[0] == '-e':
        do_method = lambda n: encrypt_file(n, n.rsplit('.', 1)[0])
        sys.argv.pop(0)
